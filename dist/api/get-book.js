...
export async function getBook(isbn13) {
    ...
    try {
        ...
        const bookContext = JSON.parse(bookContainer.find("script[type=application/ld+json]").text());
        ...
        const pubDate = bookContainer
            .find("p[data-testid=publicationInfo]")
            .text()
            .trim()
            .replace("First published ", "") || null;
        const description = bookContainer
            .find("div.BookPageMetadataSection__description .DetailsLayoutRightParagraph__widthConstrained")
            .text() || null;
        const cover = bookContainer.find(".BookCover__image>div>img")?.attr("src") || null;
        ...
        const genres = () => {
            ...
            const genresList = genresContainer.find("span.BookPageMetadataSection__genreButton");
        ...
        console.error(`Couldn't fetch book with ISBN number ${isbn13}. (The ISBN number may be invalid). \nError:`, error.message);
    }
    finally {
...
//# sourceMappingURL=get-book.js.map