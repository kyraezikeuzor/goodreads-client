...
export async function getBookshelf(username, shelfType) {
    ...
    try {
        ...
        books = await Promise.all(booksOnPage.map(async (_, element) => {
            const isbn13 = document(element).find("td.isbn13 div.value").text().trim() || "";
            let book;
            let readAt;
            if (isbn13.length > 1) {
        ...
        console.error(`Couldn't fetch ${shelfType} bookshelf from ${username}. \nError:`, error.message);
    }
    finally {
...
//# sourceMappingURL=get-bookshelf.js.map